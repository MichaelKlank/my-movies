        ...
        match state.tmdb_service.search_movies(t, None).await {
            Ok(results) => results
                ...
                .map(|m| TmdbSearchResult {
                    ...
                    poster_url: m.poster_path.as_ref().map(|p| {
                        my_movies_core::services::TmdbService::poster_url(p, "w200")
                    }),
...
pub async fn search_tmdb_movies(
    ...
    match state.tmdb_service.search_movies(&params.query, params.year).await {
        Ok(results) => {
